...
from console import clear, set_idle_timer_disabled
...
class Compass (Scene):
	def setup(self):
		global r, compass, compass_angle, closed, norise
		norise = 0
		r = 150
		circle = ui.Path.oval(0, 0, r*2, r*2)
		compassdot = ui.Path.oval(0, 0, r/14, r/14)
		circle.line_width = 5
		compassdot.line_width = 4
		shadow = ('yellow', 0, 0, 15)
		self.face = ShapeNode(circle, 'white', 'orange', shadow=shadow)
		self.add_child(self.face)
		k = 0
		for i in range(0, 4):
			label = LabelNode(str(k), font=('ArialBold', 0.085*r))
			label.color = 'yellow'
			a = k*(pi/180)
			label.position = sin(a)*(r*1.085), cos(a)*(r*1.06)
			self.face.add_child(label)
			k += 90
		self.sunpos = []
		sunpos = [(r*0.99, 3, 'red'), (r*0.99, 3, 'red'), (r*0.99, 3, 'red'), (r*0.99, 3, 'green')]
		for l, w, color in sunpos:
			shape = ShapeNode(ui.Path.rounded_rect(0, 0, w, l, w/2), color)
			shape.anchor_point = (0.5, 0)
			self.sunpos.append(shape)
			self.face.add_child(shape)
		self.face.add_child(ShapeNode(ui.Path.oval(0, 0, 10, 10), 'black')) # Zentrum
		compass = ShapeNode(compassdot, 'yellow', 'red', shadow=shadow)
		self.face.add_child(compass)
		self.face.position = 190,600
		
		self.label2 = LabelNode(str(text1), font=('ArialBold', 0.1*r))
		self.label2.color = 'white'
		self.label2.position = 75,75
		self.label2.anchor_point = (0, 0)
		self.add_child(self.label2)
		
		self.label3 = LabelNode(str(text2), font=('ArialBold', 0.1*r))
		self.label3.color = 'green'
		self.label3.position = 75,40
		self.label3.anchor_point = (0, 0)
		self.add_child(self.label3)
		
		self.label4 = LabelNode(str(text3), font=('ArialBold', 0.1*r))
		self.label4.color = 'white'
		self.label4.position = 75,25
		self.label4.anchor_point = (0, 0)
		self.add_child(self.label4)
		
		self.label5 = LabelNode(str(text4), font=('ArialBold', 0.1*r))
		self.label5.color = 'red'
			
		self.label5.anchor_point = (0,0)
		self.add_child(self.label5)
		
		self.label6 = LabelNode(str(compass_angle)[0:6], font=('ArialBold', 0.15*r))
		self.label6.color = 'white'
		self.label6.position = 275,750
		self.label6.anchor_point = (0,0)
		self.add_child(self.label6)
		
		self.label7 = LabelNode('X', font=('ArialBold', 0.15*r))
		self.label7.color = 'grey'
		self.label7.position = 25,750
		self.label7.anchor_point = (0,0)
		self.add_child(self.label7)
		
		self.label8 = LabelNode('N', font=('ArialBold', 0.15*r))
		self.label8.color = 'white'
	...
	def update(self):
		...
		if len(self.label5.text) > 26:
			self.label5.position = 75, -10
		else:
			self.label5.position = 75, 25
		
		self.label2.text = text1
		self.label3.text = text2
		self.label4.text = text3			
						
		if norise == 0:
			
			self.sunpos[0].rotation = sunrise_val*pi/180
			self.sunpos[1].rotation = midday_val*pi/180
			self.sunpos[2].rotation = sunset_val*pi/180
...
global closed, horiginal
closed = 0
horiginal = ''

def drawsunstatus(rise, noon, set, current, h):
	global sunrise_val, midday_val, sunset_val, current_val, compass_angle, text1, text2, text3, text4, heading
	...
	if compass_angle > 337.25:
			heading = 'N'
			
	if compass_angle >= 0 and compass_angle <= 22.5:
		heading = 'N'
			
	if compass_angle > 292.5 and compass_angle <= 337.25:
		heading = 'NW'
...
def addcurrent():
	global x1, x2, x3, x4, label2, label3, label4, currentDT, a1, a2, a3
	
	currentDTnow = datetime.datetime.now()
	
	if currentDT.hour == 23 and currentDTnow.hour == 0:
	...
	ch = currentDTnow.strftime("%H")
	ch = float(ch)
	cm = currentDTnow.strftime("%M")
	cm = float(cm)
	
	label2 = ''
	label4 = ''

	label2 += 'Now	(Hour, Direction, Height)' + '\n'
	sonne(ch,cm)
	x4 = str(bq)
	y = str(bt)
	label2 += currentDTnow.strftime("%H:%M") + '		' + x4[0:5] + '		' + y[0:5] + '\n'
	#label3 += "------------------------------" + '\n'
		
	if currentDTnow < sunset and currentDTnow > sunrise:
		restu = sunset - currentDTnow
		resta = currentDTnow - sunrise
		#label += '▼ in ' + str(rest)
		label4 += 'Sunrise since	' + str(resta)[0:10] + '\n'
		label4 += 'Sunset in		' + str(restu)[0:10] + '\n'
			
	if currentDTnow < sunrise:
		rest = sunrise - currentDTnow
		#label += '▲ in ' + str(rest)
		label4 += 'Sunrise in	' + str(rest)[0:10]
			
	if currentDTnow > sunset:
		rest = currentDTnow - sunset
		#label += '▼ seit ' + str(rest)
		label4 += 'Sunset since	' + str(rest)[0:10]

def zeiten():
	pi=3.14159265359

	global ah, am, mh, mm, uh, um, ahx, mhx, uhx, norise, label, x1, x2, x3, ch, cm, sunrise, sunset, midday, currentDT, label3
	...
	label += 'Sunrise	(Hour, Direction, Height)' + '\n'
	sonne(ah,am)
	x1 = str(bq)
	y = str(bt)
	...
	label += "------------------------------" + '\n'
	label += 'Sunset	(Hour, Direction, Height)' + '\n'
	sonne(uh,um)
	x3 = str(bq)
	y = str(bt)
...
clear()
on_main_thread(set_idle_timer_disabled)(flag)
...
if devtype == 'iPad':
	print('This app runs on iPhones only.')
	exit()
